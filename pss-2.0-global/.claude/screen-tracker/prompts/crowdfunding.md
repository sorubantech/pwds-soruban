...
- **Scope**: Second fix pass against the PM audit register. Blocker **CF-B3** — `"GoalMet"` status was never assigned anywhere, so an `AutoClose` campaign kept accepting money past its configured goal forever, and GoalMet→Closed/milestone paths were unreachable.
- **Fix**:
  - **CF-B3** — the *consumer* side was already correct: the donate gate (`InitiateCrowdFundDonation.cs:200-210`) treats `GoalMet`+`AutoClose` as **not accepting** and `GoalMet`+`KeepAccepting`/`ShowStretchGoal` as **still accepting**. The only gap: nothing wrote `"GoalMet"`. Added `ApplyCrowdFundGoalMetAsync` to `ResolveOnlineDonationStaging` — because deferred promotion means the junction insert there is where new crowdfund money is first recognized. After a promoted crowdfund donation, if refund-net raised (`NetAmount - RefundedAmount` through the junction, matching CF-B2) ≥ `GoalAmount` **and** status is `Active`/`Published`, it flips → `GoalMet` via `ExecuteUpdateAsync`. Race-safe (the UPDATE re-asserts the Active/Published predicate), tracker-free (safe alongside the handler's detach block), best-effort (own swallowing try/catch — a goal-met failure never rolls back or fails the already-committed donation). `AutoClose` now genuinely stops; `CloseCrowdFund` (Active|GoalMet → Closed) is now reachable.
- **Files touched**:
  - BE: `OnlineDonationInbox/Commands/ResolveOnlineDonationStaging.cs` (one call site + one private helper)
  - FE / DB: none
- **Deviations from spec**: None — completes a lifecycle the spec/entity already described (`PageStatus` enum + `GoalExceededBehavior` semantics were already defined; only the writer was missing).
- **Deferred (intentionally NOT done)**: **GoalReached/milestone email** stays a `SERVICE_PLACEHOLDER` — every crowdfund donor email (confirmation/receipt in `ConfirmCrowdFundDonation`) is a stubbed log today; wiring only this one live via the `EmailSendJob` infra would be inconsistent and out of scope. **GoalMet auto-revert on later refund** (raised drops back below goal) is a separate product decision, not part of CF-B3.
- **Verification**: `dotnet build Base.Application.csproj` → Build succeeded, 0 errors (546 pre-existing warnings). Runtime E2E (fund an `AutoClose` campaign to its goal via inbox resolve → status flips to GoalMet → next donate attempt returns `PAGE_NOT_ACCEPTING`; `KeepAccepting`/`ShowStretchGoal` still accept after GoalMet) is user-side.
- **Known issues opened**: None.
- **Known issues closed**: CF-B3 (marked ✅ in the audit register).
- **Next step**: Remaining blockers — CF-B4 anti-abuse (⚠ shared wiring across P2P/ODP, needs coordination on `Base.API/DependencyInjection.cs`) and CF-B1 deferred-promotion (product decision). Then guards (CF-H2/H3), donor-facing FE (CF-H7/8/9/10), CF-H4 FX pass.

### Session 12 — 2026-07-10 — FIX (PM audit anti-abuse batch — self-contained slice of CF-B4) — PARTIAL (BE+FE build clean)

- **Scope**: Blocker **CF-B4** (anti-automation stack is a no-op — 5 mechanisms). Fixed the 2 of 5 that are self-contained (no shared-wiring edits, no infra provisioning); documented the other 3 as needing a coordinated infra/wiring pass. User delegated both the scope and the idempotency-store decisions ("you can decide and go with proper flow" / "you can decide") — chose the self-contained slice + in-process `IMemoryCache`.
- **Blast-radius finding (read-only scoping first, as promised before touching shared files)**: the 3 public Initiate handlers (Crowdfund / P2P / ODP) are field-for-field identical in their anti-abuse BE code. But the FE honeypot was **not** uniformly broken — only Crowdfund hardcoded `website:""`; ODP (`state.honeypot`) and P2P (`website` state) already transmit it. And ODP has **no** IdempotencyKey on its Initiate path at all.
- **Fix**:
  - **CF-B4 (4) Honeypot** — Crowdfund `donate-form.tsx:304` was sending `website: ""` even though it renders a bound hidden input (`:586`) and has a client-side `if (honeypot) return` (`:270`). Changed to `website: honeypot` so the *server-side* check (already correct in `InitiateCrowdFundDonation.cs:97-108`) actually fires — defense in depth. ODP/P2P unchanged (already correct).
  - **CF-B4 (5) Idempotency** — added an entry guard (after honeypot/CSRF/reCAPTCHA, before tenant resolution/gateway construction) to `InitiateCrowdFundDonation` + `InitiateP2PDonation`: `IMemoryCache.TryGetValue($"{surface}-donate-idem:{IdempotencyKey}")` → on hit, replay the first built response; on the success path, `cache.Set(..., 10-min TTL)`. Prevents a retry (network timeout / double-click / gateway re-submit) from creating a **new** gateway order / Razorpay plan+subscription. Crowdfund handler needed `IMemoryCache` added to its ctor + a `using`; P2P already had `cache` injected (used for leaderboard invalidation).
- **Files touched**:
  - BE: `CrowdFunds/Commands/InitiateCrowdFundDonation.cs` (using + ctor `IMemoryCache` + entry guard + cache-on-success), `Public/PublicMutations/InitiateP2PDonation.cs` (entry guard + cache-on-success)
  - FE: `public/crowdfundingpage/donate-form.tsx` (1 line — `website: honeypot`)
  - DB: none (in-process cache — no schema change, per the delegated store decision)
- **Deviations from spec**: None — hardens existing security stubs the spec already described; no new fields, no schema, no shared-wiring edits.
- **Deferred (intentionally NOT done — needs infra/wiring the user owns)**: **(1) Rate-limit binding** — GraphQL is one endpoint; `app.MapGraphQL()` has no `.RequireRateLimiting` and the partition key reads `Request.Query["slug"]` while GraphQL sends slug in the POST body. A true fix needs a per-operation guard (HotChocolate request interceptor) + edits to shared `Base.API/DependencyInjection.cs` — affects all 6 public pages (crowdfund/P2P/ODP/prayer/volunteer/event), so it's a coordinated pass, not a screen-local fix. **(2) reCAPTCHA** hardcoded `1.0m` — needs a Google reCAPTCHA secret + outbound verify call. **(3) CSRF** shape-only — needs antiforgery middleware / cookie-bound token. **ODP idempotency** — no IdempotencyKey on its Initiate path (DTO+GraphQL+FE); guard it after the key is plumbed. **In-process cache caveat**: covers sequential retries on one instance, not cross-instance or concurrent-double-submit races — a distributed cache or a DB unique key on IdempotencyKey would close that (schema/infra = user-owned).
- **Verification**: `dotnet build Base.Application.csproj` → Build succeeded, 0 errors (548 pre-existing warnings). Runtime E2E (submit the same donation twice with one IdempotencyKey → second returns the identical PaymentSessionId/order, no duplicate staging row / gateway order; fill the hidden `website` field on the Crowdfund form → BE silently mock-succeeds with no writes) is user-side.
- **Known issues opened**: None new (residuals documented in the audit register's CF-B4 Session-12 note).
- **Known issues closed**: None fully — CF-B4 marked ⚠️ PARTIAL in the register (2 of 5 mechanisms fixed).
- **Next step**: CF-B4 remainder is a coordinated infra/shared-wiring pass (rate-limit + reCAPTCHA + CSRF + ODP idempotency-key) — needs user go-ahead before editing `Base.API/DependencyInjection.cs`. Otherwise: CF-B1 deferred-promotion (product decision), then guards (CF-H2/H3), donor-facing FE (CF-H7/8/9/10), CF-H4 FX pass.

### Session 13 — 2026-07-10 — FIX (PM audit lifecycle-guards batch — CF-H2 + CF-H3) — COMPLETED (BE build clean)

- **Scope**: Highs **CF-H2** (delete/unpublish guards ignore staged money) + **CF-H3** (publish server-gate weaker than the advisory validator). Both screen-owned BE, no shared wiring, no schema — the self-contained highs I flagged as the momentum-preserving next step after CF-B4. User: "you can decide and continue."
- **Fix**:
  - **CF-H2** — `DeleteCrowdFund` and `UnpublishCrowdFund` guarded only the `CrowdFundDonations` junction, which is empty under deferred-promotion until staff resolve the inbox row. So a campaign that had *already captured* public money (a COMPLETED `OnlineDonationStaging` row, no junction yet) could be deleted / pulled back to Draft. Both handlers now also count COMPLETED (`PAYMENTSTATUS` = COMPLETED/SUCCESS, resolved from `MasterDatas`) staging rows for the `CrowdFundId` and add them to the junction count before the guard decides. Error message reports the combined total.
  - **CF-H3** — the rich publish ERROR ruleset (category, reserved-slug via `CrowdFundSlugValidator.IsReserved`, stretch-goal, WhatsApp-template, impact-row, milestone-row, plus the shape-tolerant `HasAnyEnabledPaymentMethod`) lived only in the advisory `ValidateCrowdFundForPublishQuery`; the authoritative `PublishCrowdFund` mutation carried a thinner hand-rolled copy and could be called directly to bypass them. **Root cause = duplication, so the fix is de-duplication:** extracted all ERROR rules into a new pure static `CrowdFundPublishValidation.CollectErrors(entity, donationPurposeValid, nowUtc)` under `Base.Application/Validations/`. The query handler now delegates its ERROR rules to it (keeps its own WARNING rules); the Publish handler calls it and throws `BadRequestException` with the joined messages on any violation. Single source of truth → the pre-publish modal and the server gate can never disagree again.
- **Files touched**:
  - BE: `Validations/CrowdFundPublishValidation.cs` (NEW — shared ERROR ruleset + `HasAnyEnabledPaymentMethod`), `CrowdFunds/Commands/DeleteCrowdFund.cs` (staging count guard), `CrowdFunds/Commands/UnpublishCrowdFund.cs` (staging count guard), `CrowdFunds/Commands/PublishCrowdFund.cs` (call shared validator instead of inline list), `CrowdFunds/Queries/ValidateCrowdFundForPublish.cs` (delegate ERROR rules to shared helper; removed its private `HasAnyEnabledPaymentMethod` + now-unused `System.Text.Json` import)
  - FE: none
  - DB: none
- **Deviations from spec**: None — tightens existing guards to match the deferred-promotion money model and the already-documented publish ruleset; no new fields, no schema, no shared-wiring edits. The new `CrowdFundPublishValidation` is a pure static helper in the feature's own `Validations/` folder (not a DI/DbContext/routes wiring file).
- **Verification**: `dotnet build Base.Application.csproj` → Build succeeded, 0 errors (548 pre-existing warnings, unchanged). Runtime E2E (donate on a live campaign so a COMPLETED staging row exists → Delete/Unpublish now rejects with the donation count; call the Publish mutation directly on a campaign missing a category / with a reserved slug / with WhatsApp-alerts-on-but-no-template → now rejected identically to the modal) is user-side.
- **Known issues opened**: None.
- **Known issues closed**: CF-H2, CF-H3 (both ✅ in the audit register).
- **Next step**: Remaining self-contained highs are FE (CF-H7/H8/H9/H10 donor-facing) and CRM (CF-H11/H12). Money-truth CF-H4 (FX) and the CF-B4 shared-wiring remainder still need, respectively, a cross-currency test pass and user go-ahead on `Base.API/DependencyInjection.cs`. CF-B1 deferred-promotion is still the open product decision.

### Session 16 — 2026-07-10 — FIX (PM audit CRM-cluster batch — CF-H11 + CF-H12) — COMPLETED (FE tsc clean)

- **Scope**: #16-owned Highs **CF-H11** (CRM screen largely orphaned — lifecycle/quick-edit modals built but never mounted; two dead drawer "Edit" buttons) + **CF-H12** (KPI tiles + chip counts never refetch after a mutation). User chose "wire the DataTable + delete orphans" over mounting the pre-built card-grid. *(Sibling #173-owned CF-H7 — dead cover-fees toggle, removed — was done in the same session and is logged in the audit register.)*
- **Fix**:
  - **CF-H11** — the `LifecycleConfirmModal` (Publish/Unpublish/Close/Archive/Duplicate) and `DeleteCrowdFundModal` were fully built and store-driven (`openModal` → mutation → `bumpRefresh` → `closeModal`) but never mounted, so the drawer buttons calling `openModal("edit")` were dead and no lifecycle path existed from the list. Mounted both modals in the router `index.tsx`; added a status-aware **Lifecycle** section to the drawer (`crowdfund-detail-sheet.tsx`) — Draft→Publish/Duplicate/Delete, Published→Close/Unpublish/Duplicate, Active·GoalMet→Close/Duplicate, Closed→Archive/Duplicate — each opening the mounted confirm modal (BE re-validates every transition). Fixed the dead header "Edit" button to route to the full editor (`router.push(editHref)`) and removed the dead "Quick Edit Basic" ActionRow.
  - **CF-H12** — the KPI widgets already refetched on the store `refreshToken`, but the index-page chip-count query (a separate `useQuery`) and the grid rows did not, so a mutation left them stale. Wired `index-page.tsx` to, on `refreshToken` bump, call the FlowDataTable store's `setRefresh(true)` (reload grid) **and** `refetchSummary()` (chips). All lifecycle/delete mutations now flow through the mounted modals → one `bumpRefresh` refreshes grid + widgets + chips + drawer together. Also **disabled the native row Delete** (`enableDelete: false`) and moved Delete into the drawer (Draft-only, store-driven) — the native delete reloaded only the grid and had lost the "Draft-only" affordance (partially closes CF-M10).
- **Files touched**:
  - BE: none
  - FE: `crowdfunding/index.tsx` (mount `LifecycleConfirmModal` + `DeleteCrowdFundModal`), `crowdfunding/index-page.tsx` (refreshToken → grid `setRefresh` + chip `refetchSummary`; `enableDelete: false`), `crowdfunding/crowdfund-detail-sheet.tsx` (status-aware Lifecycle section; fixed dead Edit button; removed dead Quick Edit Basic row); **deleted** `crowdfunding/crowdfund-quick-edit-dialog.tsx` (no importers)
  - DB: none
- **Deviations from spec**: The index-page comment claimed lifecycle "lives in the editor's action pill, exactly like the Event screen" and "no bespoke lifecycle modals" — this session intentionally reintroduces list-surface lifecycle via the drawer (the built modals) because CF-H11 found admins had no publish/close path from the list. `crowdfund-card.tsx` was **NOT** deleted despite being unused by the row-table layout — it is still registered in the shared `card-variant-registry.ts` (via `card-grid/variants/crowdfund-card.tsx` + `types.ts`), so deleting it would require shared-wiring edits with cross-screen ripple; left in place as an unused-but-registered card variant.
- **Verification**: `npx tsc --noEmit` → 0 errors (whole FE). Runtime E2E (open a campaign's drawer from the grid → Publish/Close/etc. → confirm modal fires, toast, and grid + KPI tiles + chip counts all update without a reload; Draft campaign shows Delete, non-Draft doesn't) is user-side.
- **Known issues opened**: None.
