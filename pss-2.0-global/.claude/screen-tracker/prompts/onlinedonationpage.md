...
### Session 36 — 2026-07-10 — AUDIT + FIX — COMPLETED

- **Scope**: Full PM-level read-only audit of screen #10 (admin setup + public NAV/IFRAME + donation pipeline + 3 gateways + staging/inbox coupling) via 5 parallel dimension reviews, then applied the 3 self-contained **P0 quick-wins** that needed no shared-wiring, no infra provisioning, and no blueprint change. Full findings register (≈40 findings, `ODP-*` ids, mapped to existing ISSUEs) written to a sibling file — see below.
- **Findings register**: [onlinedonationpage-audit-2026-07-10.md](../bug-reports/onlinedonationpage-audit-2026-07-10.md) — 9 P0 / 14 P1 / 11 P2 / 8 LOW. Verdict: **NO-GO for live donations** until the P0 gate closes; money-path cryptography is solid, but anti-abuse, receipts/promotion, consent, multi-currency, i18n, and non-India gateways are stubbed/absent.
- **Files touched**:
  - BE: `Base.API/Controller/PaymentWebhookController.cs` — injected `IHostEnvironment`; gated the anonymous `braintree/{companyCode}/test` endpoint to `IsDevelopment()` (returns 404 elsewhere). **[ODP-B2]**
  - FE: `presentation/components/page-components/setting/publicpages/onlinedonationpage/sections/identity-section.tsx` — rewrote `handleTitleChange`: auto-derives slug only while `status = Draft` && not manually edited, and reserved-slug-checks the derived value; stops the title-edit-rewrites-published-slug link-rot bug. **[ODP-H5]**
  - DB: `sql-scripts-dyanmic/online-donation-page-sqlscripts.sql` — new STEP 0a2 (`PAYMENTMETHOD/PENDING` sentinel) + STEP 0b (`DONATIONMODE/OD`, `DONATIONTYPE/{ONETIMEDONATION,RECURRINGDONATION}`, `PAYMENTSTATUS/{PENDING,COMPLETED,FAILED}`, `RECURRINGFREQUENCY/{MO,QT,SA,AN}`), all NOT EXISTS-guarded. **[ODP-B6]** — **seed = user applies**.
- **Deviations from spec**: None (all three are in-scope fixes). Note the audit surfaced that the Session-15 staging-first pivot itself deviates from approved §⑫ ("donations live in GlobalDonations") — tracked as ODP-B5, a product decision, not fixed here.
- **Known issues opened**: register-local `ODP-B1..B9 / H1..H14 / M1..M11 / L1..L8` (see sibling file). No new prompt-level `ISSUE-*` minted (register supersedes for this audit).
- **Known issues closed / advanced**:
  - **ISSUE-28** (MASTERDATA_MISSING) → **PARTIAL** — donation-path MasterData folded into the idempotent seed (ODP-B6). Inbox-promotion MasterData (CONTACTBASETYPE/EMAILTYPE/CONTACTSTATUS) owned by #175, still out of scope.
  - **ODP-B2** (anonymous test-webhook financial-record forgery; overlaps the ISSUE-24 config-key drift exposure) — CLOSED. Also blocks the ISSUE-24 vulnerable path in prod.
- **Build verification**:
  - BE: `dotnet build Base.API.csproj` → **0 errors**, 18 warnings. PASS.
  - FE: `pnpm tsc --noEmit` → **0 new errors** in `identity-section.tsx`. PASS.
  - Runtime not exercised — user applies the updated seed, then smoke-tests: (a) a fresh-tenant first donation no longer throws MASTERDATA_MISSING; (b) editing a Published page's title leaves its slug untouched; (c) `POST /api/webhooks/braintree/{code}/test` returns 404 in Staging/Prod.
- **Next step (recommended P0 batch, user to prioritise)**: ODP-H1 (make `WritePaymentEvent` unconditional across Braintree/Razorpay — self-contained BE, good next fix); then the shared-wiring/infra pass for ODP-B1 (rate-limit binding + reCAPTCHA secret + CSRF middleware) and the ODP-B3 concurrency migration; ODP-B4/B5/B7/B8 are product/blueprint decisions → `/plan-screens`.
...
### Session 39 — 2026-07-10 — ODP-M8 MasterData NULL-filter (CONTINUED FIX)
- **Trigger**: /continue-screen #10 — take the next self-contained BE fix. The remaining P0s are blocked (B3 = user-owned migration / no sibling guard to mirror; B4/B5/B7/B8 = product decisions → /plan-screens; B9 = E2E creds), so picked a fully-closable money-path correctness bug: ODP-M8.
- **Root cause**: `GetMasterDataId(typeCode, dataValue)` filtered `m.IsDeleted == false`. `IsDeleted` is `bool?` and seeds insert MasterData with `IsDeleted = NULL`; Postgres `"IsDeleted" = false` **excludes NULL rows**, so the lookup returned 0 and the gateway webhook's status-transition silently no-op'd (recurring donation status never advanced).
- **Change** (4 BE files, mirrors the proven `GetMasterDataIdFirstOf` which deliberately omits the filter): dropped `&& m.IsDeleted == false` from the MasterData helper at `PayUWebhookController.cs:287`, `RazorpayWebhookController.cs:438`, `PaymentWebhookController.cs:216`, and **`PaymentFlowService.cs:94`** (same defect on the Initiate path; `GetMasterDataIdOptional` delegates to it — audit's 3-controller scope missed this 4th site). PayU's own comment already said "do NOT filter == false" — code now matches the comment.
- **Scope note**: FULL closure of ODP-M8 (not partial). Left the `CompanyPaymentGateways … IsDeleted == false` filters alone — different entity, genuinely soft-deleted with explicit `false`.
- **Build verification**: BE `dotnet build Base.API.csproj` → **0 errors** (78 warnings, pre-existing). PASS. A phantom "GetCrowdFundPublicStatsQuery no 2-arg ctor" error was an unrelated in-flight CrowdFund working-tree edit + stale `Base.Application.dll`; rebuilding Base.Application (0 errors) cleared it — not this fix's file, not staged/touched by me.
- **Next step**: ODP-M9 (slug-immutability guard reads GlobalDonations but donations live in staging — BE validator, self-contained) or ODP-H8/H9 (FE/CSP, self-contained); the reCAPTCHA+CSRF shared-platform pass remains the big cross-cutting item.

### Session 38 — 2026-07-10 — ODP-B1 rate-limit binding (CONTINUED FIX)
- **Trigger**: /continue-screen #10 — take the next self-contained BE fix. Chose ODP-B1's rate-limit half (the only anti-abuse sub-part that is self-contained + has a proven reuse pattern; reCAPTCHA/CSRF are cross-cutting).
- **Root cause (narrow)**: the `"DonationSubmit"` policy the ODP resolvers referenced was **never registered**. All six sibling public pages (P2P, CrowdFund, Prayer, Volunteer, EventReg) already register + bind their own policy via ASP.NET `AddRateLimiter` + `[EnableRateLimiting]`; ODP was the lone page missing the policy row and the attribute. Not a GraphQL-architecture blocker after all — the pattern was already established.
- **Change** (2 BE files, pure reuse mirroring `CrowdFundDonationSubmit`):
  - `Base.API/DependencyInjection.cs` — added `options.AddPolicy("DonationSubmit", …)`, FixedWindow 5/min, partition `online-donate-{ip}-{slug}`.
  - `Base.API/.../OnlineDonationPagePublicQueries.cs` — added `using Microsoft.AspNetCore.RateLimiting;` + `[EnableRateLimiting("DonationSubmit")]` on **both** `InitiateOnlineDonation` and `ConfirmOnlineDonation`; rewrote the stale SERVICE_PLACEHOLDER comment.
- **Scope note (honest, PARTIAL)**: closes the *rate-limit* half of ODP-B1 only. **Still open (org-wide, NOT ODP-specific):** reCAPTCHA=1.0 in every public Initiate handler (needs FE reCAPTCHA-v3 token + request-DTO field + user-owned Google secret + shared `siteverify` service) and shape-only CSRF (needs cookie-pair middleware). Both should be built once as shared platform services, not per-page patches. Register finding + Fix Log updated to PARTIAL.
- **Known limitation (carried from sibling pattern, not introduced here)**: partition reads `Request.Query["slug"]`, empty for GraphQL POST-body variables → effective partition is per-IP (still 5/min/IP). Every sibling policy shares this; mirrored deliberately for consistency.
- **Build verification**: BE `dotnet build Base.API.csproj` → **0 errors** (643 warnings, pre-existing). PASS. Runtime not exercised.
- **Next step**: the reCAPTCHA + CSRF shared-platform pass (cross-cutting, user-owned secrets); or ODP-H1's recurring-webhook ledger half (blocked on ODP-B5).

### Session 37 — 2026-07-10 — ODP-H1 audit-parity (CONTINUED FIX)
- **Trigger**: /continue-screen #10 — take the recommended next self-contained BE fix (ODP-H1).
- **Change**: `ConfirmOnlineDonation.cs` — the donor Confirm path previously wrote `audit.AuditLogs` rows only on the PayU branch. Added 5 `WritePaymentEvent` calls mirroring the existing PayU pattern so **every** one-time gateway branch is audited:
  - Braintree — sale exception (`PAYMENT_FAILED`/HIGH), processor decline (`PAYMENT_FAILED`/MEDIUM), capture (`PAYMENT_CAPTURED`/LOW).
  - Razorpay (one-time) — verify-fail (`PAYMENT_FAILED`/MEDIUM), capture (`PAYMENT_CAPTURED`/LOW).
  - `auditLogWriter` was already injected/in-scope (PayU used it); no DI or signature change.
- **Scope note (honest, PARTIAL)**: this closes the *one-time Confirm* audit gap only. The other half of ODP-H1 — recurring-cycle webhooks (Braintree `SUBSCRIPTION_CHARGED_SUCCESSFULLY`, Razorpay `subscription.charged`) not creating `GlobalDonation` rows — is **still open**; it lives in the webhook controllers and depends on the staging→ledger promotion decision (ODP-B5). Register finding + Fix Log updated to PARTIAL.
- **Build verification**: BE `dotnet build Base.API.csproj` → **0 errors**. PASS. Runtime not exercised (no card sandbox run this session).
- **Next step**: recurring-webhook ledger/receipt half of ODP-H1 (blocked on ODP-B5 promotion decision); then the ODP-B1 anti-abuse shared-wiring/infra pass.
