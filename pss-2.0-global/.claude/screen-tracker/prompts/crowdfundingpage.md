...
### Session 11 — 2026-07-02 — FIX (public donate form disabled for Published campaigns) — COMPLETED (FE)

- **Symptom**: public page showed "Donations are no longer accepted for this campaign." even inside the `[StartDate, EndDate]` window. Root cause: `canDonate` accepted **only** `pageStatus === "Active"`, so a `Published` (or transient pre-start Published — see Session 10, Publish sets `Published` then auto-advances to `Active` only when StartDate ≤ now) page never enabled the form. User rejected a data-patch SQL `UPDATE` as improper: the correct flow is Publish → Published → form enables.
- **Fix (FE-only, no SQL/no BE)**: made `canDonate` **date-aware** and treat `Published` + `Active` as equivalent live states — donations open when a live page `hasStarted && !hasEnded` (computed from `startDate`/`endDate`), plus the existing `GoalMet && !AutoClose` case. Kept the edge banner coherent: the "not started yet" `Published` banner now shows **only** when `!hasStarted` (future start date); a started Published campaign is live with no contradictory banner. Matches P2P parity (gates only on `Closed`).
- **Files touched — FE**: `page-content.tsx` (date-aware `canDonate`, passes `hasStarted` to `<EdgeBanner>`), `edge-banners.tsx` (`hasStarted?` prop, gated Published banner, last-days warning extended to started Published). **No BE / no migration / no seed.** **Next step**: Publish `build-school` (start date already past) → form enables; no `?_tenant=` needed on localhost (Session-10 dev fallback).

### Session 12 — 2026-07-02 — FIX/ENHANCE (rename page-template MasterData code CROWDFUNDPAGETEMPLATE → CROWDFUNDINGPAGETEMPLATE) — COMPLETED (FE) / SEED + BE-comment HANDOFF

- **Ask**: user asked whether the crowdfunding template picker was (wrongly) using the **event** code `EVENTREGPAGETEMPLATE`, and to switch it to `CROWDFUNDINGPAGETEMPLATE`. **Finding**: it was NOT using the event code — the whole stack consistently used its own `CROWDFUNDPAGETEMPLATE` (event code appeared only in a doc-comment "mirrors the EVENTREGPAGETEMPLATE pattern"). The only delta from the requested code was `CROWDFUND` vs `CROWDFUND`**`ING`**. User confirmed the rename (matches the `crowdfundingpage` screen/folder name).
- **Change**: renamed the MasterData TypeCode `CROWDFUNDPAGETEMPLATE` → `CROWDFUNDINGPAGETEMPLATE` across every layer. Seed gains a **STEP 0 idempotent rename** (`UPDATE ... SET TypeCode` guarded by old-exists + new-not-exists) so an already-seeded type + its `CrowdFund.PageTemplateId` FKs migrate in place without orphaning; fresh installs fall through to the existing `WHERE NOT EXISTS` inserts.
- **Files touched — FE**: `components/page-template-picker.tsx` (`TEMPLATE_MD_TYPECODE` + comment), `domain/entities/donation-service/CrowdFundDto.ts` (doc-comment). **DB seed**: `sql-scripts-dyanmic/CrowdFundingPage-page-template.sql` (all refs + STEP 0 rename). **BE (comment-only, no build impact)**: `Base.Domain/.../CrowdFund.cs` (two `<summary>` doc-comments). **No migration** (FK column unchanged). **Next step**: user re-runs `CrowdFundingPage-page-template.sql` — STEP 0 renames any existing type, then the picker query returns rows under the new code and selection persists a real `masterDataId`.

### Session 13 — 2026-07-02 — ENHANCE (premium public page + "Meet Our Beneficiaries" + showcase config) — COMPLETED (FE) / BE BUILD + 1 MIGRATION HANDOFF

- **Ask**: make the 5 page templates look professional/premium (reference: HopeBridge STANDARD donation page — hero + donate widget + "Your Impact" checklist + "Where Your Donation Goes" donut + trust badges + impact-stats bar + "Meet Our Beneficiaries" cards). Crowdfunding/P2P are beneficiary-support focused → include beneficiary details + config like `ShowBeneficiaries`. **Scope chosen** (AskUserQuestion): beneficiary content = **admin-authored jsonb** (1 new column); redesign = **STANDARD-first, others as variants**.
- **Key finding**: the public renderer *ignored* `pageTemplateCode` — all 5 templates rendered one fixed 2-col layout. Now `page-content.tsx` branches on a `TEMPLATE_LAYOUTS` config: STANDARD = full premium layout; STORY_FOCUS (wider prose, no side widgets), IMAGE/VIDEO_FOCUS (variants), MINIMAL (strips premium chrome). Each premium block gates on BOTH its `enabledSectionsJson` toggle AND the template flag.
- **Minimized BE surface — ONE new column** (`CrowdFund.BeneficiariesJson` jsonb, `[{name,photoUrl,quote,program}]`). Everything else reuses existing data: "Your Impact" checklist + "Where Your Donation Goes" donut ← `impactBreakdownJson`; impact-stats bar ← real aggregates (raised/donors/goal%/days); trust badges = static chrome. 4 new config toggles are **data-level** in `EnabledSectionsJson` (`beneficiaries/trustBadges/impactStats/yourImpact`, default ON) — no columns.
- **Files touched — BE (build + 1 migration)**: `Base.Domain/.../CrowdFund.cs` (+`BeneficiariesJson`), `Base.Infrastructure/.../CrowdFundConfiguration.cs` (jsonb map), `Schemas/.../CrowdFundSchemas.cs` (ResponseDto + PublicDto + PageUpdateRequest), `Commands/UpdateCrowdFundPage.cs` (persist), `Queries/GetCrowdFundById.cs` + `GetCrowdFundBySlug.cs` (projections).
- **Files touched — FE**: DTOs `CrowdFundDto.ts` (+`beneficiariesJson`, `CrowdFundBeneficiaryEntry`+`parseBeneficiaries`) & `CrowdFundingPageDto.ts` (public DTO + update req + 4 `CrowdFundEnabledSections` toggles); GQL `CrowdFundPublicQuery.ts`+`CrowdFundQuery.ts` (+field); store blank + `editor-page.tsx` `toUpdateRequest`; editor `components/section-toggles.tsx` + `components/beneficiaries-editor.tsx` (NEW) + `tabs/page-builder-tab.tsx` (new section card + 3 display toggles) + `components/live-preview.tsx` (trust chips / stats / beneficiaries reflections); **NEW public components** `public/crowdfundingpage/{trust-badges,impact-stats-bar,your-impact-list,donation-donut,beneficiaries-showcase}.tsx` + barrel + rewired `page-content.tsx`.
- **Deviations from spec**: extends the Spec (new column + sections) — approved by user via AskUserQuestion. **Next step**: user builds BE + creates migration for `fund.CrowdFunds.BeneficiariesJson` (nullable jsonb); then admins add beneficiaries in Page Builder → "Meet Our Beneficiaries" card, toggle the 4 new premium sections in Display Options. **P2P parity is a FOLLOW-ON** (mirror this onto p2pcampaignpage next).
