...
> ### Fix Log — Session 45 (2026-07-13, /continue-screen — ODP-M11 dead legacy Braintree path removed, BE compile clean)
> - ✅ **ODP-M11 / ISSUE-26 (FIXED)** — Deleted the entire dead appsettings-bound legacy Braintree credential path, which coexisted with (and was fully superseded by) the LIVE per-tenant-creds path (`Base.Support/.../BraintreeProvider` via `PaymentGatewayFactory`). Removed 5 files: the singleton impl `Base.Infrastructure/Services/BraintreeService.cs` (the only `using Braintree;` consumer in that project); the interface + DTOs `Base.Application/.../DonationBusiness/PaymentGateways/IBraintreeService.cs` (`BraintreePaymentRequest`/`BraintreeTransactionResult`); the never-bound config POCO `.../PaymentGateways/BraintreeSettings.cs`; and the two `[ExtendObjectType]` GraphQL surfaces `Base.API/.../Donation/Queries/BraintreePaymentQueries.cs` (`GetBraintreeClientToken`) + `.../Mutations/BraintreePaymentMutations.cs` (`ProcessBraintreePayment`/`VerifyBraintreeTransaction`/`RefundBraintreeTransaction`). Removed the `<PackageReference Include="Braintree" />` from `Base.Infrastructure.csproj` and the dead `"Braintree"` section from `Base.API/appsettings.json`. **Verified dead before deletion:** no `AddSingleton<IBraintreeService>`/`Configure<BraintreeSettings>`/`GetSection("Braintree")` registration anywhere, zero FE callers, DTOs referenced only in the deleted files. **Left untouched:** `Base.Support.csproj`'s own `Braintree VersionOverride="5.28.0"` package (live path needs it). Post-deletion grep of all 8 removed symbols → 0 references. **Compile proof:** `Base.Infrastructure` (transitively `Base.Application`) → 0 errors; `Base.API` → 0 CS errors (only 8 × MSB3021/MSB3027 output-copy locks from the running `Base.API` process — environment locks, not compile errors). No FE change. **Owner:** BE cleanup.
>
> ### Fix Log — Session 44 (2026-07-13, /continue-screen — ODP-M10 config-key drift, VERIFIED already-resolved, no code change)
> - ✅ **ODP-M10 (VERIFIED FIXED — no code change)** — The config-key drift (`PaymentGateway:EncryptionKey` vs canonical `PaymentGateway:CredentialEncryptionKey`) no longer exists anywhere in the backend. A whole-repo grep for `EncryptionKey` returns only `Security:EncryptionKey` and the canonical `PaymentGateway:CredentialEncryptionKey`; the drifted string is present in **no** `.cs`/config file. The TEST webhook path (`TestBraintreeWebhook`, `PaymentWebhookController.cs:228`) decrypts tenant creds through `_encryptionService.DecryptForCompany` — the **same** method as the production webhook (lines 71-72) — which funnels through `DerivePerCompanyKey` reading only the canonical key (`EncryptionService.cs:128`). The drift was eliminated incidentally when the v2 (AES-GCM/HKDF) refactor consolidated all decryption onto `DecryptForCompany`. Verify-and-close; nothing to build.
>
> ### Fix Log — Session 43 (2026-07-13, /continue-screen — ODP-H8 clickjacking boundary, FE tsc clean)
> - ✅ **ODP-H8 (FIXED)** — The NAV/IFRAME framing security boundary now actually exists. Before this, `frame-ancestors` lived only as TODO comments in `(public)/layout.tsx` / `embed/[slug]/page.tsx` / `iframe-widget.tsx` and **no header was emitted anywhere** — so the first-party NAV donor page `/p/{slug}` was fully framable (open to clickjacking overlay attacks). Added `async headers()` to `next.config.mjs` with two DISJOINT locale-prefixed rules (middleware rewrites every path to include `{lang}` before render, so `source` must include the `:lang` segment):
>   - `/:lang/p/:slug*` → `Content-Security-Policy: frame-ancestors 'none'` **+** legacy `X-Frame-Options: DENY` (never embeddable).
>   - `/:lang/embed/:slug*` → `Content-Security-Policy: frame-ancestors *` (the widget is INTENDED to be embedded on any charity site); `X-Frame-Options` deliberately omitted (it can't express "allow all origins", and leaving it unset lets `frame-ancestors *` govern).
>   Only the `frame-ancestors` directive is set — a full CSP (`script-src` etc.) is out of scope and would risk breaking existing inline scripts/styles. `headers()` chosen over middleware because the `auth()` wrapper makes response-header injection there awkward, and per-path matching is clean. Synced the stale ISSUE-8 TODO comments in `layout.tsx` + `embed/[slug]/page.tsx`. Replaced the skipped e2e stub with a real 2-case `test.describe("clickjacking boundary")` that fetches both surfaces and asserts the emitted CSP / X-Frame-Options headers. FE `npx tsc --noEmit` → 0 errors.
>
...
- **ODP-H6 · Entire SEO/OG/robots editor card missing.** `ogTitle/ogDescription/ogImageUrl/robotsIndexable` are stored but have no admin control; `robotsIndexable` is also ignored on the public page (`p/[slug]/page.tsx:119` hardcodes `index:true`). **Owner:** admin FE + public route. — ✅ **FIXED (session 46)**; new store-driven `seo-section.tsx` admin card (ogTitle/ogDescription/ogImageUrl + share-card preview + robotsIndexable Switch) wired into `editor-page.tsx` for both NAV & IFRAME. Public half: extended public query + public DTO to carry `robotsIndexable`, then `p/[slug]/page.tsx generateMetadata` now emits `robots: { index/follow: !!data.robotsIndexable }` instead of hardcoded true. FE tsc 0 errors.
...
- **ODP-H8 · No `frame-ancestors` CSP anywhere.** NAV `/p/{slug}` has no clickjacking protection; the NAV/IFRAME security boundary doesn't exist. **Owner:** `next.config.mjs` / middleware. **Maps-to:** ISSUE-8. — ✅ **FIXED (session 43)**; `next.config.mjs headers()` emits `frame-ancestors 'none'` + `X-Frame-Options: DENY` on `/:lang/p/:slug*` and `frame-ancestors *` on `/:lang/embed/:slug*`. e2e stub unskipped with real header assertions. FE tsc 0 errors.
...
- **ODP-M10 · ISSUE-24 config-key drift** — ✅ **VERIFIED FIXED (session 44, no code change)** — the drifted `PaymentGateway:EncryptionKey` no longer exists anywhere; TEST + prod webhooks both decrypt via `DecryptForCompany` → `DerivePerCompanyKey`, reading only the canonical `PaymentGateway:CredentialEncryptionKey` (`EncryptionService.cs:128`). Eliminated incidentally by the v2 AES-GCM/HKDF consolidation. **Owner:** BE config. *(ODP-B2's env-gate also blocks the TEST path in prod.)*
- **ODP-M11 · ISSUE-26 Dead legacy Braintree credential path** — ✅ **FIXED (session 45)** — Deleted the appsettings singleton `BraintreeService` + its `IBraintreeService` interface/DTOs + never-bound `BraintreeSettings` + the `GetBraintreeClientToken` query & 3 dead mutations, plus the `Base.Infrastructure` Braintree package ref and the `"Braintree"` appsettings section. Verified dead (no DI registration, no FE caller, no lingering symbol refs); live per-tenant `BraintreeProvider` path untouched. BE compile clean (only running-API output-copy locks). **Owner:** BE cleanup.
