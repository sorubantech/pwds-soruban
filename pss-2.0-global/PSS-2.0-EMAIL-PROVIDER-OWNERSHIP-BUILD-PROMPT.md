...
| 2026-08-07 | §④.1 | **No-op.** Mapster's convention mapping already carried `IsPlatformProvider` onto `CompanyEmailProviderResponseDto`; nothing was missing in the read path. The "screen is always OWN" symptom was purely §⑥.1 — the field was absent from every FE gql selection set. | Done |
| 2026-08-07 | §④.2 ★ | `SensitiveFieldMasking.RedactPlatformCredentials(dto)` added to `GetCompanyEmailProviderStats.cs`; called from all three read handlers — active (`GetCompanyEmailProviderStats.cs:54`), by-id (`GetCompanyEmailProviderById.cs:47`), list (`GetCompanyEmailProvider.cs:66`, `foreach` over the DTOs in place because `GridFeatureResult.Data` is get-only). Applied in the **handler**, not the resolver. | Done |
| 2026-08-07 | §④.3 ★ | Credential resolution moved into `EmailProviderConfigRepository` (injects `IPlatformCommunicationProviderResolver`; copies the `ops` row's `ProviderConfiguration`/`WebhookUrl`/`WebhookSecret` onto a platform row at send time — one central swap, so every send path inherits it). `EmailSenderService` fails closed with `EmailProviderNotConfiguredException("Platform email sending is not configured. Contact support.")` when the resolved config is empty. **No `appsettings` fallback.** Metering call untouched (INV-8). | Done |
| 2026-08-07 | §④.4 / §④.5 | `UsePlatformEmailProvider` (idempotent) and `UseOwnEmailProvider` added. Neither takes `IsPlatformProvider` from client input — it is set server-side in both. | Done |
| 2026-08-07 | §④.6 | **Design deviation, deliberate.** The prompt asked that `SaveCompanyEmailProvider` let the From name/email through on a platform row. Not implementable as written: `SaveCompanyEmailProviderValidator` demands provider-shape JSON that `"{}"` can never satisfy, and the request has no `companyId` with which to detect a platform target. `Save`/`Update`/`Delete`/`ActivateDeactivate` therefore **refuse** platform rows outright (`PlatformEmailProviderGuard.EnsureNotPlatform`), and the editable carve-out is served by `usePlatformEmailProvider` plus the existing `EmailSendingIdentity` mutations. Net effect on the tenant is identical; the write surface is narrower. | Done |
| 2026-08-07 | §②.3 ★ | `PlatformSendingIdentityRule.EnsureFromEmailAllowedAsync` enforces the `@<platform domain>` suffix server-side. While wiring it, found and closed an adjacent hole the prompt did not name: sending-identity **Create/Update** never validated `FromEmail` against the parent provider's `SendingDomainName` at all, so a platform tenant could have set an arbitrary From address via the identity dialog and bypassed the lock entirely. | Done |
| 2026-08-07 | §⑤ / Q2 | No `EMAILPROVIDER`/`SENDGRID` MasterData rows existed anywhere → `sql-scripts-dyanmic/email-provider-ownership-masterdata-seed.sql` written (122 lines, idempotent). **USER-OWNED — not applied.** Q2 **dissolved**: the sending domain is not a constant to be decided, it is derived at runtime from the `ops` default EMAIL row's `DefaultFromEmail` via `PlatformEmailDomain.From(...)`. | Done |
| 2026-08-07 | §④.7 | Mutations `usePlatformEmailProvider` / `useOwnEmailProvider`; query resolver `GetPlatformEmailSenderInfo` → field `platformEmailSenderInfo` returning `{ displayName, sendingDomainName, isAvailable }`. No credential ever crosses that boundary. | Done |
| 2026-08-07 | §⑥.1–⑥.6 | `isPlatformProvider` added to all three gql selection sets; `platform-sender-card.tsx`, `plan-allowance-panel.tsx`, `email-mode-switch-dialog.tsx` created and barrelled. Cards **hidden, never disabled** in PLATFORM mode (provider picker, credentials, webhook, DNS, throttle, reputation, SMTP, status banner, sticky save bar); Sending Identities kept in both modes; allowance panel rendered in **both** modes above the throttle card. Card 5 retitled "Provider Throttling" with the replaced helper line and a non-blocking `overThrottleWarning`. Mode switch invalidates the provider query **and** `myCommunicationUsage`. | Done |
| 2026-08-07 | §⑧ | Items 2, 13, 14, 17 verified in code. Item 15: `npx tsc --noEmit --incremental false` **exit 0**; ESLint on the touched folder = 0 errors (1 pre-existing `_removed` warning). Items 1, 3–12 need a running DB/API and are **not** demonstrated — see the handover note below. Item 16 is the user's to run. | Partial |

### Handover — what is not yet demonstrated

The code is complete and typechecks. The following acceptance items are runtime and **remain open**:

- **#1 must be checked first.** Migration `20260805112547_Add_IsPlatformProvider_ToCompanEmailProvider.cs` exists on disk, but no DB connection was made — confirm the column is actually **applied** in the target database before anything else is tested.
- **#3–#12** need the API up: playground reads, the `LIKE '%SG.%'` → 0 rows assertion, a live PLATFORM send, the fail-closed send with the `ops` row deactivated, the two metering assertions, the off-domain rejection, and the OWN → PLATFORM → OWN round trip.
- `sql-scripts-dyanmic/email-provider-ownership-masterdata-seed.sql` must be applied, or `usePlatformEmailProvider` has no `SENDGRID` MasterData to resolve.
- `EmailProviderConfigRepository` gained a constructor parameter — check its DI registration at `Base.API/DependencyInjection.cs:125` still resolves when you build.

### Open questions still unanswered

**Q4** (does a new tenant land in PLATFORM automatically — the prompt recommends yes but says "confirm before building", and it would add a step to `ProvisionTenantCommand`, so **nothing was built for it**), **Q5** (do the SENDGRID MasterData rows exist per environment — the seed is written but unapplied), **Q6** (exceed-allowance behaviour in PLATFORM mode — metering blocks today; unchanged).
